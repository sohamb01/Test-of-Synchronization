...
X_1 <- read_csv("Data_For_Sec_6_1/Vicksberg2324 - Sheet1.csv")
X_2 <- read_csv("Data_For_Sec_6_1/Tennesi2324 - Sheet1.csv")
...
colnames(tot_dat)<-c("Vicksburg, MS (downstream from Memphis)", "Memphis, TN")
...
ggplot(my_data_long, aes(x =date_range, 
  ...
  labs(title = "",
       x = "Dates",
       y = "Water Discharge, cubic feet per second",
       color = "Legend Title")+
