...
SyncBootstage2<-function(X, n, d, nboot=5000, B_n=floor(n^{1/4})){
  ...
  for(j in 1:d){
    indiv_cusum[,j]<- abs(cumsum(X[,j]-mean(X[,j])))
  }
  ...
  T_n<- sum(sapply(1:d, function(j){max(indiv_cusum[,j])- indiv_cusum[common_tau, j]}))/sqrt(n) # test statistic
  ...
  for ( B in 1:nboot) {
    ...
    for (j in 1:d) {
      cusum_count[B,j]<- (max(indiv_cusum[,j]) >= max(abs(cumsum((Z[[B]])[,j]-mean((Z[[B]])[,j])))))
    }
  }  
  ...
  for(B in 1:nboot){
    ...
    for(j in 1:d){
      indiv_cusum_boot[,j]<- abs(cumsum(Z[[B]][,j]-mean(Z[[B]][,j])))
    }
    ...
    T_n_boot[B]<- sum(sapply(1:d, function(j){max(indiv_cusum_boot[,j])-
        indiv_cusum_boot[common_tau_boot, j]}))/sqrt(n) # test statistic
